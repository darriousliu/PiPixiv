...
class NovelRecommendedPagingSource : PagingSource<String, Novel>() {
    override suspend fun load(params: LoadParams<String>): LoadResult<String, Novel> {
        return try {
            val resp = if (params.key.isNullOrEmpty()) {
                PixivRepository.getNovelRecommended(
                    filter = Filter.ANDROID,
                    includeRankingNovels = true,
                    includePrivacyPolicy = false
                )
            } else {
                PixivRepository.loadMoreNovelRecommended(params.key?.queryParams ?: emptyMap())
            }
            val novels = if (requireUserPreferenceValue.isR18Enabled) {
                (resp.novels + resp.rankingNovels).distinctBy { it.id }
            } else {
                (resp.novels + resp.rankingNovels).distinctBy { it.id }.filterNormalNovel()
            }
            LoadResult.Page(
                data = novels,
                prevKey = params.key,
